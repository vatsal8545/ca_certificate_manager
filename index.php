...
<body>
      ...
      <div class="top-grid">
        ...
        <section class="panel card side-panel">
          ...
          <div class="form-grid">
            ...
            <label>Place of Signing *<select id="placeInput">
      ...
      <section class="panel card table-panel">
        <div class="panel-header">
          ...
          <div class="import-controls">
            <input id="clientImportInput" type="file" accept=".csv,.xlsx,.xls" hidden>
            <button class="primary small" id="importClientsBtn" type="button">Import Excel/CSV</button>
          </div>
        </div>
        <div id="importStatus" class="import-status">Import clients from Excel or CSV using columns like Client Name, Status, PAN Number, GST Number, Financial Year, Amount.</div>
  ...
  <script>
    ...
    const clientImportInput = document.getElementById('clientImportInput');
    ...
    function normalizeHeader(value) {
      ...
      const headerMap = {
        ...
        'financial years checked': 'financialYear',
    ...
    function parseImportedRows(workbook) {
      ...
      return rows
        .map(row => {
          ...
          Object.keys(row).forEach(key => {
            normalizedRow[normalizeHeader(key)] = normalizeValue(row[key]);
          });
    ...
    function buildClientFromImportRow(row) {
      const name = row.clientName || row.name || '';
      const status = row.status || 'Proprietor';
      const pan = row.pan || '';
      const gst = row.gst || '';
      const yearCandidates = [row.financialYear, row.year].filter(Boolean);
      const amountCandidates = [row.amount].filter(Boolean);
      const years = [];

      if (yearCandidates.length && amountCandidates.length) {
        yearCandidates.forEach(yearValue => {
          const rawYears = String(yearValue).split(/[;,|]/);
          rawYears.forEach(item => {
            const year = item.trim();
            if (year) {
              years.push({ year, amount: amountCandidates[0] });
            }
          });
        });
      }
    ...
    function mergeImportedClients(importedRows) {
      const clients = loadClients();
      importedRows.forEach((row, index) => {
        const importedClient = buildClientFromImportRow(row);
        if (!importedClient.name || !importedClient.pan) {
          return;
        }

        const existingIndex = clients.findIndex(item => {
          const samePan = importedClient.pan && item.pan && item.pan.toLowerCase() === importedClient.pan.toLowerCase();
          const sameName = importedClient.name && item.name && item.name.toLowerCase() === importedClient.name.toLowerCase();
          return samePan || sameName;
        });
        ...
        if (existingIndex >= 0) {
          ...
          if (importedClient.years.length) {
            ...
            importedClient.years.forEach(yearData => {
              const exists = existingClient.years.some(item => item.year === yearData.year && item.amount === yearData.amount);
              if (!exists) {
                existingClient.years.push(yearData);
              }
            });
          }
        } else {
          clients.unshift({
            ...
            status: importedClient.status || 'Proprietor',
            pan: importedClient.pan,
            gst: importedClient.gst,
            years: importedClient.years
          });
        }
      });
      return clients;
    ...
    async function importClientsFromFile(file) {
      ...
      if (!['csv', 'xls', 'xlsx'].includes(extension || '')) {
        showImportStatus('Please upload a CSV or Excel file.', true);
        return;
      }

      try {
        showImportStatus('Reading the selected file...');
        const arrayBuffer = await file.arrayBuffer();
        const workbook = XLSX.read(arrayBuffer, { type: 'array' });
        const importedRows = parseImportedRows(workbook);
        if (!importedRows.length) {
          showImportStatus('No usable rows were found in the file.', true);
          return;
        }

        const combinedClients = mergeImportedClients(importedRows);
        saveClients(combinedClients);
        showImportStatus(`Imported ${importedRows.length} row(s) successfully.`);
        if (clientImportInput) {
          clientImportInput.value = '';
        }
      } catch (error) {
        console.error(error);
        showImportStatus('The file could not be imported. Please check the format and try again.', true);
      }
    }
    ...
    function createYearRow(year = '', amount = '') {
      ...
      for (let i = currentYear; i >= currentYear - 15; i--) {
        ...
        yearOptions.push(`<option value="${fy}" ${fy === year ? 'selected' : ''}>${fy}</option>`);
      }
      
      row.innerHTML = `
        <select class="year-field">
      ...
      row.querySelector('.year-field').addEventListener('change', updateSummary);
    ...
    function renderClientsTable() {
      ...
      clientsTableBody.innerHTML = clients.map(client => {
        const yearsWithAmounts = client.years.map(item => `${item.year} (₹${Number(item.amount).toLocaleString('en-IN')})`).join(', ');
        return `
          <tr>
            <td>${client.name}</td>
            <td>${client.status}</td>
    ...
    if (importClientsBtn) {
      importClientsBtn.addEventListener('click', () => clientImportInput?.click());
    }
    if (clientImportInput) {
      clientImportInput.addEventListener('change', event => {
        const file = event.target.files?.[0];
        if (file) {
          importClientsFromFile(file);
        }
      });
    }
